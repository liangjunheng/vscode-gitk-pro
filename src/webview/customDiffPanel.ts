...
(function() {
  ...
  // 细化一个替换行对：小片段使用 LCS，超出预算保守降级为整行。
  function wordDiff(original, modified) { const left = String(original).match(/\\s+|[A-Za-z0-9_$]+|[^\\sA-Za-z0-9_$]/g) || []; const right = String(modified).match(/\\s+|[A-Za-z0-9_$]+|[^\\sA-Za-z0-9_$]/g) || []; if (!left.length || !right.length || left.length * right.length > 12000) return { left: escapeHtml(original), right: escapeHtml(modified) }; const matrix = Array.from({ length: left.length + 1 }, function() { return new Uint16Array(right.length + 1); }); for (let leftIndex = left.length - 1; leftIndex >= 0; leftIndex--) for (let rightIndex = right.length - 1; rightIndex >= 0; rightIndex--) matrix[leftIndex][rightIndex] = left[leftIndex] === right[rightIndex] ? matrix[leftIndex + 1][rightIndex + 1] + 1 : Math.max(matrix[leftIndex + 1][rightIndex], matrix[leftIndex][rightIndex + 1]); let leftIndex = 0; let rightIndex = 0; let leftHtml = ''; let rightHtml = ''; while (leftIndex < left.length || rightIndex < right.length) { if (leftIndex < left.length && rightIndex < right.length && left[leftIndex] === right[rightIndex]) { const token = escapeHtml(left[leftIndex]); leftHtml += token; rightHtml += token; leftIndex++; rightIndex++; } else if (rightIndex >= right.length || (leftIndex < left.length && matrix[leftIndex + 1][rightIndex] >= matrix[leftIndex][rightIndex + 1])) leftHtml += '<span class="word-removed">' + escapeHtml(left[leftIndex++]) + '</span>'; else rightHtml += '<span class="word-added">' + escapeHtml(right[rightIndex++]) + '</span>'; } return { left: leftHtml, right: rightHtml }; }
    ...
    while (index < result.left.length) {
      ...
      if (unchanged) {
        ...
        for (let rowIndex = index; rowIndex < end; rowIndex++) {
          ...
          if (currentLeft.type === 'removed' || currentLeft.type === 'modified-removed') unifiedRemoved += lineHtml(currentLeft.number, '', currentLeft.value, currentLeft.type, '−', change, currentLeft.html);
          if (currentRight.type === 'added' || currentRight.type === 'modified-added') unifiedAdded += lineHtml('', currentRight.number, currentRight.value, currentRight.type, '+', change, currentRight.html);
