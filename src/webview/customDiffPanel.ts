...
(function() {
  ...
  function wordDiff(original, modified) { const left = Array.from(String(original)); const right = Array.from(String(modified)); if (!left.length || !right.length || left.length + right.length >= 500) return { left: escapeHtml(original), right: escapeHtml(modified) }; const matrix = Array.from({ length: left.length + 1 }, function() { return new Uint16Array(right.length + 1); }); for (let leftIndex = left.length - 1; leftIndex >= 0; leftIndex--) for (let rightIndex = right.length - 1; rightIndex >= 0; rightIndex--) matrix[leftIndex][rightIndex] = left[leftIndex] === right[rightIndex] ? matrix[leftIndex + 1][rightIndex + 1] + 1 : Math.max(matrix[leftIndex + 1][rightIndex], matrix[leftIndex][rightIndex + 1]); const leftChanged = new Array(left.length).fill(false); const rightChanged = new Array(right.length).fill(false); let leftIndex = 0; let rightIndex = 0; while (leftIndex < left.length || rightIndex < right.length) { if (leftIndex < left.length && rightIndex < right.length && left[leftIndex] === right[rightIndex]) { leftIndex++; rightIndex++; } else if (rightIndex >= right.length || (leftIndex < left.length && matrix[leftIndex + 1][rightIndex] >= matrix[leftIndex][rightIndex + 1])) leftChanged[leftIndex++] = true; else rightChanged[rightIndex++] = true; } function extendToWord(chars, changed) { for (let index = 0; index < chars.length; index++) { if (!changed[index]) continue; const range = subwordRange(chars, index); for (let current = range[0]; current < range[1]; current++) changed[current] = true; } } function render(chars, changed, className) { let html = ''; let index = 0; while (index < chars.length) { const isChanged = changed[index]; let end = index + 1; while (end < chars.length && changed[end] === isChanged) end++; const text = escapeHtml(chars.slice(index, end).join('')); html += isChanged ? '<span class="' + className + '">' + text + '</span>' : text; index = end; } return html; } extendToWord(left, leftChanged); extendToWord(right, rightChanged); return { left: render(left, leftChanged, 'word-removed'), right: render(right, rightChanged, 'word-added') }; }
  ...
  function calculateDiff(original, modified) {
    ...
    const removedBlocks = collectPureBlocks(rowsLeft, 'removed'); const addedBlocks = collectPureBlocks(rowsRight, 'added'); const matchedAdded = new Set(); function similarity(first, second) { const firstTokens = new Set(first.lines.join('\\n').toLowerCase().split(/[^\\p{L}\\p{N}_$]+/u).filter(function(token) { return token.length >= 3; })); const secondTokens = new Set(second.lines.join('\\n').toLowerCase().split(/[^\\p{L}\\p{N}_$]+/u).filter(function(token) { return token.length >= 3; })); let shared = 0; firstTokens.forEach(function(token) { if (secondTokens.has(token)) shared++; }); const total = firstTokens.size + secondTokens.size - shared; const lengthRatio = Math.min(first.lines.join('').length, second.lines.join('').length) / Math.max(first.lines.join('').length, second.lines.join('').length); return total ? shared / total * lengthRatio : 0; } removedBlocks.forEach(function(removedBlock) { let bestIndex = -1; let bestScore = .90; addedBlocks.forEach(function(addedBlock, index) { if (matchedAdded.has(index) || addedBlock.change === removedBlock.change) return; const exact = addedBlock.lines.length === removedBlock.lines.length && addedBlock.lines.every(function(line, lineIndex) { return line === removedBlock.lines[lineIndex]; }); const score = exact ? 1 : similarity(removedBlock, addedBlock); if (score > bestScore) { bestScore = score; bestIndex = index; } }); if (bestIndex < 0) return; const addedBlock = addedBlocks[bestIndex]; matchedAdded.add(bestIndex); for (let index = removedBlock.start; index < removedBlock.end; index++) rowsLeft[index].type = 'moved-removed'; for (let index = addedBlock.start; index < addedBlock.end; index++) rowsRight[index].type = 'moved-added'; });
    ...
    while (index < result.left.length) {
      ...
      if (unchanged) {
        ...
        for (let rowIndex = index; rowIndex < end; rowIndex++) {
          ...
          if (currentLeft.type === 'moved-removed') leftClass = 'moved-block'; else if (currentLeft.type === 'removed') leftClass = 'removed-block'; else if (currentLeft.type === 'modified-removed') leftClass = 'modified-block';
          if (currentRight.type === 'moved-added') rightClass = 'moved-block'; else if (currentRight.type === 'added') rightClass = 'added-block'; else if (currentRight.type === 'modified-added') rightClass = 'modified-block';
        }
  ...
  function renderDiff(diff, expandedRanges) {
    ...
    const addedLines = result.right.filter(function(row) { return row.type === 'added' || row.type === 'modified-added' || row.type === 'moved-added'; }).length;
    const removedLines = result.left.filter(function(row) { return row.type === 'removed' || row.type === 'modified-removed' || row.type === 'moved-removed'; }).length;
  ...
  function renderAllDiffs(diffs, expectedGeneration, expectedEpoch) {
    ...
    for (const diff of diffs) {
      ...
      try {
        ...
        holder.innerHTML = '<button class="file-header"><span class="file-header-row"><span class="status status-' + escapeHtml(diff.status) + ' status-without-stats">' + escapeHtml(diff.status) + '</span><span class="file-path">' + escapeHtml(diff.path) + '</span></span></button><div class="empty">无法渲染此文件的 Diff：' + escapeHtml(message) + '</div>';
