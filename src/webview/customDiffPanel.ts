...
export class CustomDiffPanel implements vscode.Disposable {
    ...
    private async onMessage(message: unknown): Promise<void> {
        ...
        if (payload.type !== 'selectFile' || typeof payload.path !== 'string' || payload.generation !== this.requestGeneration || !this.allowedPaths.has(payload.path)) { return; }
...
(function() {
  ...
  // 对齐策略与 VS Code 一致：行级忽略首尾空白，细化阶段保留真实文本差异。
  function lineKey(line) { return String(line).trim(); }
  function lineEquals(leftLine, rightLine) { return lineKey(leftLine) === lineKey(rightLine); }
  ...
  function wordDiff(original, modified) { const left = splitGraphemes(original); const right = splitGraphemes(modified); if (!left.length || !right.length) return { left: escapeHtml(original), right: escapeHtml(modified) }; const leftChanged = new Array(left.length).fill(false); const rightChanged = new Array(right.length).fill(false); let prefix = 0; while (prefix < left.length && prefix < right.length && left[prefix] === right[prefix]) prefix++; let suffix = 0; while (suffix < left.length - prefix && suffix < right.length - prefix && left[left.length - suffix - 1] === right[right.length - suffix - 1]) suffix++; const leftEnd = left.length - suffix; const rightEnd = right.length - suffix; const cells = (leftEnd - prefix) * (rightEnd - prefix); if (cells <= 250000) { const matrix = Array.from({ length: leftEnd - prefix + 1 }, function() { return new Uint32Array(rightEnd - prefix + 1); }); for (let leftIndex = leftEnd - 1; leftIndex >= prefix; leftIndex--) for (let rightIndex = rightEnd - 1; rightIndex >= prefix; rightIndex--) matrix[leftIndex - prefix][rightIndex - prefix] = left[leftIndex] === right[rightIndex] ? matrix[leftIndex - prefix + 1][rightIndex - prefix + 1] + 1 : Math.max(matrix[leftIndex - prefix + 1][rightIndex - prefix], matrix[leftIndex - prefix][rightIndex - prefix + 1]); let leftIndex = prefix; let rightIndex = prefix; while (leftIndex < leftEnd || rightIndex < rightEnd) { if (leftIndex < leftEnd && rightIndex < rightEnd && left[leftIndex] === right[rightIndex]) { leftIndex++; rightIndex++; } else if (rightIndex >= rightEnd || (leftIndex < leftEnd && matrix[leftIndex - prefix + 1][rightIndex - prefix] >= matrix[leftIndex - prefix][rightIndex - prefix + 1])) leftChanged[leftIndex++] = true; else rightChanged[rightIndex++] = true; } } else { for (let index = prefix; index < leftEnd; index++) leftChanged[index] = true; for (let index = prefix; index < rightEnd; index++) rightChanged[index] = true; } function extendToWord(chars, changed) { for (let index = 0; index < chars.length; index++) { if (!changed[index]) continue; const range = subwordRange(chars, index); for (let current = range[0]; current < range[1]; current++) changed[current] = true; } } function render(chars, changed, className) { let html = ''; let index = 0; while (index < chars.length) { const isChanged = changed[index]; let end = index + 1; while (end < chars.length && changed[end] === isChanged) end++; const text = escapeHtml(chars.slice(index, end).join('')); html += isChanged ? '<span class="' + className + '">' + text + '</span>' : text; index = end; } return html; } extendToWord(left, leftChanged); extendToWord(right, rightChanged); return { left: render(left, leftChanged, 'word-removed'), right: render(right, rightChanged, 'word-added') }; }
  function codeHtml(code, highlighted) { return code === '' ? '&nbsp;' : (highlighted === undefined ? escapeHtml(code) : highlighted); }
  function lineHtml(leftNumber, rightNumber, code, type, marker, change, highlighted) { const attribute = change === undefined ? '' : ' data-change="' + change + '"'; return '<div class="line ' + type + '"' + attribute + '><span class="line-marker">' + (marker || '') + '</span><span class="line-number">' + (leftNumber || '') + '</span><span class="line-number">' + (rightNumber || '') + '</span><span class="line-code">' + codeHtml(code, highlighted) + '</span></div>'; }
  function splitLineHtml(number, code, type, change, highlighted) { const attribute = change === undefined ? '' : ' data-change="' + change + '"'; return '<div class="line ' + type + '"' + attribute + '><span class="line-number">' + (number || '') + '</span><span class="line-code">' + codeHtml(code, highlighted) + '</span></div>'; }
  function calculateDiff(original, modified) {
    ...
    function patienceAnchors(leftStart, leftEnd, rightStart, rightEnd) { const leftCounts = new Map(); const rightPositions = new Map(); for (let i = leftStart; i < leftEnd; i++) { const key = lineKey(left[i]); leftCounts.set(key, (leftCounts.get(key) || 0) + 1); } for (let i = rightStart; i < rightEnd; i++) { const key = lineKey(right[i]); rightPositions.set(key, rightPositions.has(key) ? -1 : i); } const candidates = []; for (let i = leftStart; i < leftEnd; i++) { const key = lineKey(left[i]); const rightIndex = rightPositions.get(key); if (leftCounts.get(key) === 1 && rightIndex !== undefined && rightIndex >= rightStart) candidates.push([i, rightIndex]); } const tails = []; const previous = new Array(candidates.length); for (let i = 0; i < candidates.length; i++) { let low = 0; let high = tails.length; while (low < high) { const middle = (low + high) >>> 1; if (candidates[tails[middle]][1] < candidates[i][1]) low = middle + 1; else high = middle; } previous[i] = low ? tails[low - 1] : -1; tails[low] = i; } const anchors = []; for (let index = tails.length ? tails[tails.length - 1] : -1; index >= 0; index = previous[index]) anchors.unshift(candidates[index]); return anchors; }
    ...
    const removedBlocks = collectPureBlocks(rowsLeft, 'removed'); const addedBlocks = collectPureBlocks(rowsRight, 'added'); const matchedAdded = new Set(); removedBlocks.forEach(function(removedBlock) { const addedIndex = addedBlocks.findIndex(function(addedBlock, index) { return !matchedAdded.has(index) && addedBlock.change !== removedBlock.change && addedBlock.lines.length === removedBlock.lines.length && addedBlock.lines.every(function(line, lineIndex) { return line === removedBlock.lines[lineIndex]; }); }); if (addedIndex < 0) return; const addedBlock = addedBlocks[addedIndex]; matchedAdded.add(addedIndex); for (let index = removedBlock.start; index < removedBlock.end; index++) rowsLeft[index].type = 'moved-removed'; for (let index = addedBlock.start; index < addedBlock.end; index++) rowsRight[index].type = 'moved-added'; });
    return { left: rowsLeft, right: rowsRight, finalNewline: finalNewline };
  }
    ...
    while (index < result.left.length) {
      ...
      if (unchanged) {
        ...
        for (let rowIndex = index; rowIndex < end; rowIndex++) {
          ...
          leftBlock += splitLineHtml(currentLeft.number, currentLeft.value, currentLeft.type, change, currentLeft.html); rightBlock += splitLineHtml(currentRight.number, currentRight.value, currentRight.type, change, currentRight.html);
          ...
          if (!currentLeft.type && !currentRight.type) unifiedRemoved += lineHtml(currentLeft.number, currentRight.number, currentLeft.value, '', '', change);
            ...
            if (currentLeft.type === 'removed' || currentLeft.type === 'modified-removed' || currentLeft.type === 'moved-removed') unifiedRemoved += lineHtml(currentLeft.number, '', currentLeft.value, currentLeft.type, '−', change, currentLeft.html);
            if (currentRight.type === 'added' || currentRight.type === 'modified-added' || currentRight.type === 'moved-added') unifiedAdded += lineHtml('', currentRight.number, currentRight.value, currentRight.type, '+', change, currentRight.html);
          }
