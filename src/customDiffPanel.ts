...
(function() {
  ...
  function navigateChange(direction) {
    ...
    let currentIndex = lastChangeTarget ? targets.indexOf(lastChangeTarget) : -1;
    if (currentIndex < 0) {
    ...
    lastChangeTarget = target; updateNavigationButtons(targets, nextIndex);
    const toolbarHeight = document.getElementById('toolbar').offsetHeight; const diff = target.closest('.diff'); const index = diff && Number(diff.dataset.index); const file = files[index];
    ...
    window.scrollTo({ top: Math.max(0, target.getBoundingClientRect().top + window.scrollY - (window.innerHeight - targetHeight) / 2), behavior: 'auto' });
  }
  ...
  layoutButton.addEventListener('click', function() { unified = !unified; lastChangeTarget = null; updateLayout(); updateNavigationButtons(changeTargets(), -1); });
  document.addEventListener('click', function(event) {
    const element = event.target instanceof Element ? event.target : null; const code = element && element.closest('.line-code');
    if (!code) return;
    const line = code.closest('.line'); if (!line || line.classList.contains('placeholder')) return;
    ...
    const codeRect = code.getBoundingClientRect(); let offset = 8;
