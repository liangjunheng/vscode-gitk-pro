...
import { CommitFile } from './gitLogProvider';
...
export class CustomDiffPanel implements vscode.Disposable {
    ...
    async show(rootUri: vscode.Uri, hash: string, files: CommitFile[], revealPath?: string): Promise<void> {
        const isSameCommit = this.panel && this.rootUri?.toString() === rootUri.toString() && this.hash === hash;
        if (!isSameCommit) {
    ...
    private async readAllDiffs(): Promise<DiffPayload[]> {
        ...
        return this.files.map((file, index) => {
            ...
            return {
                ...
                original: original || '',
                modified: modified || '',
                error: missing ? `无法读取 Git 对象：${missing}` : undefined,
            };
        });
    }
...
(function() {
  ...
  function lineHtml(leftNumber, rightNumber, code, type, marker) { return '<div class="line ' + type + '"><span class="line-marker">' + (marker || '') + '</span><span class="line-number">' + (leftNumber || '') + '</span><span class="line-number">' + (rightNumber || '') + '</span><span class="line-code">' + (code === '' ? '&nbsp;' : escapeHtml(code)) + '</span></div>'; }
  function splitLineHtml(number, code, type) { return '<div class="line ' + type + '"><span class="line-number">' + (number || '') + '</span><span class="line-code">' + (code === '' ? '&nbsp;' : escapeHtml(code)) + '</span></div>'; }
  function calculateDiff(original, modified) {
    ...
    const rowsLeft = []; const rowsRight = []; let i = 0; let j = 0;
    while (i < left.length || j < right.length) {
      ...
      for (let row = 0; row < paired; row++) { rowsLeft.push({ value: removed[row].value, number: removed[row].number, type: 'modified-removed' }); rowsRight.push({ value: added[row].value, number: added[row].number, type: 'modified-added' }); }
      for (let row = paired; row < removed.length; row++) { rowsLeft.push({ value: removed[row].value, number: removed[row].number, type: 'removed' }); rowsRight.push({ value: '', number: '', type: 'placeholder' }); }
      for (let row = paired; row < added.length; row++) { rowsLeft.push({ value: '', number: '', type: 'placeholder' }); rowsRight.push({ value: added[row].value, number: added[row].number, type: 'added' }); }
    }
  ...
  function revealDiffTarget(target) { const viewportTop = 105; const header = target.querySelector('.file-header'); if (!header) return; suppressViewportSyncUntil = performance.now() + 200; window.scrollBy({ top: header.getBoundingClientRect().top - viewportTop, behavior: 'auto' }); }
  ...
  function renderRows(result, expandedRanges) {
    ...
    while (index < result.left.length) {
      ...
      if (unchanged) {
        ...
        leftRows += splitLineHtml(left.number, left.value, left.type); rightRows += splitLineHtml(right.number, right.value, right.type);
        if (left.type === 'removed') unifiedRows += lineHtml(left.number, '', left.value, 'removed', '−');
        else if (right.type === 'added') unifiedRows += lineHtml('', right.number, right.value, 'added', '+');
        else if (left.type === 'modified-removed') { unifiedRows += lineHtml(left.number, '', left.value, 'removed', '−'); unifiedRows += lineHtml('', right.number, right.value, 'added', '+'); }
        index++;
      }
    }
  ...
  function renderDiff(diff, expandedRanges) {
    ...
    if (diff.error) { holder.innerHTML = '<div class="empty">无法加载文件内容：' + escapeHtml(diff.error) + '</div>'; return; }
    const result = calculateDiff(diff.original, diff.modified); const oldLabel = diff.status === 'A' ? '/dev/null' : (diff.oldPath || diff.path); const newLabel = diff.status === 'D' ? '/dev/null' : diff.path; const rows = renderRows(result, expandedRanges || new Set());
    ...
    holder.querySelector('.file-header').addEventListener('click', function() { selectFile(diff.path, false); vscode.postMessage({ type: 'selectFile', path: diff.path }); });
